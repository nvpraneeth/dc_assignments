...
Supports both shared memory and message passing architectures.
"""
...
def init_shared_objects(num_nodes: int, slots_per_node: int = 1):
    ...
    LEGACY: For shared memory architecture (deprecated in favor of message passing).
    """
...
def init_message_passing_objects(num_nodes: int):
    """
    Initialize message passing infrastructure for MPLS protocols.
    
    Returns:
        (mp_manager, shared_stats) where:
        - mp_manager: MessagePassingManager instance
        - shared_stats: Shared dictionary for statistics (coordinator collects)
    """
    ...
    return mp_manager, shared_stats
