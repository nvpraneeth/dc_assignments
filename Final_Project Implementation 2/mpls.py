...
MESSAGE PASSING VERSION: Uses explicit send/receive instead of shared memory
"""
...
from message_passing import (
    MessagePassingManager, MessageType,
    send_model_state, receive_model_state
)
...
def select_smart_peer(rank: int, num_nodes: int, model: LeNet5, 
                     candidate_set_size: int, mp_manager: MessagePassingManager) -> int:
    """
    Smart peer selection: Query candidates via message passing and select best match
    Returns selected peer ID
    
    MESSAGE PASSING VERSION: Uses MODEL_REQUEST messages to query candidates
    """
    ...
    # MESSAGE PASSING: Query candidates via MODEL_REQUEST messages
    similarities = []
    for candidate_id in candidates:
        # Send request for model metadata
        mp_manager.send(rank, candidate_id, MessageType.MODEL_REQUEST, 
                       metadata={'request_type': 'metadata'})
        
        # Receive model state from candidate (they respond with their current model)
        candidate_model_state = receive_model_state(mp_manager, rank, 
                                                   sender=candidate_id, timeout=2.0)
...
def worker_process(rank: int, num_nodes: int, r_max: int, epochs: int, 
    ...
    """
    Worker process for MPLS (Message Passing Version)
    
    Handles MODEL_REQUEST messages for peer selection queries
    """
    device = 'cpu'
    clock = HybridClock()
    random.seed(rank + 42)
    ...
    for round_num in range(1, r_max + 1):
        ...
        # Smart peer selection (includes metadata query overhead via message passing)
        ...
        # MESSAGE PASSING: Exchange partial models with selected peer
        logger.synchronization("Send", f"Sending partial model to p{selected_peer} via message queue")
        send_model_state(mp_manager, rank, selected_peer, partial_state,
                        metadata={'round': round_num, 'partial_layers': partial_layers})
        logger.debug(f"Partial model sent to p{selected_peer} via message passing")
        
        # MESSAGE PASSING: Receive partial model from selected peer (blocking)
        logger.synchronization("Receive", f"Waiting for partial model from p{selected_peer} via message queue")
        peer_partial_state = receive_model_state(mp_manager, rank, sender=selected_peer, timeout=30.0)
        ...
        logger.debug(f"Received partial model from p{selected_peer} via message passing")
...
def run_mpls(num_nodes: int, r_max: int, epochs: int, bandwidth_mbps: float,
    ...
    """
    Run MPLS protocol (Message Passing Version)
    
    Uses message passing infrastructure instead of shared memory:
    - MessagePassingManager handles all inter-process communication
    - MODEL_REQUEST messages for peer selection queries
    - MODEL_STATE messages for partial model exchange
    """
    ...
    from message_passing import init_message_passing
    mp_manager = init_message_passing(num_nodes)
    
    # Statistics collection (still using shared dict for coordinator)
    ...
    for rank in range(num_nodes):
        p = mp.Process(
            ...
            args=(rank, num_nodes, r_max, epochs, bandwidth_mbps, candidate_set_size,
                  train_loaders[rank], test_loader, mp_manager,
                  shared_stats, log_dir)
        )
        p.start()
        processes.append(p)
