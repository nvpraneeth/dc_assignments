"""
Improved MPLS Handshake Implementation (Message Passing Version)
Key improvements:
...
6. Message passing instead of shared memory
"""
...
from mpls_common import prepare_data_loaders, init_message_passing_objects, create_process_logger
from mpls_synchronizer_mp import create_synchronizer_mp, SyncPhase
from message_passing import MessagePassingManager, MessageType, send_model_state, receive_model_state
from utils import (
    LeNet5,
    HybridClock,
    train_local_model,
...
def select_smart_peers_improved(
    ...
    """
    Improved multi-peer selection using gradient divergence (Message Passing Version).
    Selects multiple peers with complementary gradients (higher divergence = better).
    
    Returns:
        (selected_peers, peer_layer_divergences, divergence_messages) where peer_layer_divergences maps peer_id to layer divergences
    """
    ...
    logger.info(f"[DIVERGENCE_COMPUTE] Starting divergence computation for {len(candidates)} candidates (message passing)")
    
    # MESSAGE PASSING: Request models from candidates
    for candidate_id in candidates:
        # Send MODEL_REQUEST
        mp_manager.send(rank, candidate_id, MessageType.MODEL_REQUEST, 
                       metadata={'request_type': 'divergence_compute'})
        divergence_messages += 1
    
    # MESSAGE PASSING: Receive models from candidates
    for candidate_id in candidates:
        candidate_model_state = receive_model_state(mp_manager, rank, sender=candidate_id, timeout=2.0)
        divergence_messages += 1
...
def improved_handshake_worker(
    ...
    """
    Improved handshake worker with message passing:
    ...
    logger = create_process_logger(rank, log_dir, "MPLS-Handshake-Improved-MP")
    model = LeNet5(num_classes=10).to(device)
    
    # Initialize message passing synchronizer
    synchronizer = create_synchronizer_mp(
        rank=rank,
        num_nodes=num_nodes,
        mp_manager=mp_manager,
        logger=logger,
        timeout=30.0,  # 30 second timeout for barriers
        quorum_ratio=0.5  # 50% of nodes must be ready
    )
    ...
    logger.info(f"Improved MPLS Handshake with multi-peer aggregation and synchronization")
    ...
    logger.info(f"Synchronizer: timeout=30s, quorum=50%")
    ...
    for round_num in range(1, r_max + 1):
        ...
        # SYNC POINT 1: Wait for all nodes to complete training
        logger.info("[SYNC] Waiting for all nodes to complete training...")
        sync_success = synchronizer.wait_for_all_train(round_num)
        if not sync_success:
            logger.warning("[SYNC] Training barrier timeout, proceeding anyway")
        logger.info(f"[SYNC] Training barrier passed, ready count: {synchronizer.get_ready_count(SyncPhase.TRAIN_COMPLETE)}")
        ...
        # MESSAGE PASSING: Handle any pending MODEL_REQUEST messages from other peers
        # (for divergence computation queries)
        while True:
            try:
                message = mp_manager.receive(rank, timeout=0.01)  # Non-blocking check
                if message and message.type == MessageType.MODEL_REQUEST:
                    # Respond to request with current model state
                    current_state = copy_model_state(model)
                    send_model_state(mp_manager, rank, message.sender, current_state,
                                   metadata={'response_to_request': True})
                    logger.debug(f"Responded to MODEL_REQUEST from p{message.sender}")
                elif message is None:
                    break  # No more messages
            except:
                break  # Queue empty or timeout
        
        # Multi-peer selection using gradient divergence (message passing)
        ...
        # MESSAGE PASSING: Send partial model to all selected peers
        logger.synchronization("Send", f"Sending verified partial model to {len(selected_peers)} peers via message passing")
        for peer_id in selected_peers:
            send_model_state(mp_manager, rank, peer_id, partial_state,
                           metadata={'round': round_num, 'partial_layers': partial_layers})
        logger.debug(f"Partial models sent to {len(selected_peers)} peers via message passing")
        
        # SYNC POINT 2: Wait for all nodes to place models (send ready signal)
        logger.info("[SYNC] Waiting for all nodes to place models...")
        sync_success = synchronizer.wait_for_all_models_placed()
        if not sync_success:
            logger.warning("[SYNC] Model placement barrier timeout, proceeding anyway")
        logger.info(f"[SYNC] Model placement barrier passed")
        
        # MESSAGE PASSING: Receive partial models from all selected peers
        peer_states = {}
        for peer_id in selected_peers:
            peer_state = receive_model_state(mp_manager, rank, sender=peer_id, timeout=15.0)
            if peer_state is not None:
                # Verify peer's partial state
                peer_is_valid, peer_missing = verify_partial_state(peer_state, partial_layers)
                if peer_is_valid:
                    ...
                    peer_states[peer_id] = peer_state
                logger.info(f"[RECEIVE] Received partial model from p{peer_id} via message passing")
            else:
                logger.warning(f"[RECEIVE] Timeout waiting for partial model from p{peer_id}")
        
        if not peer_states:
            logger.error(f"Failed to receive any partial models - this should not happen with synchronization")
            # Still signal exchange complete for synchronization
            synchronizer.signal_exchange_complete()
            ...
            # Wait for all exchanges and next round
            synchronizer.wait_for_all_exchanges()
            synchronizer.wait_for_next_round()
            continue
        ...
        # SYNC POINT 3: Signal exchange complete and wait for all
        synchronizer.signal_exchange_complete()
        logger.info("[SYNC] Waiting for all nodes to complete exchanges...")
        sync_success = synchronizer.wait_for_all_exchanges()
        if not sync_success:
            logger.warning("[SYNC] Exchange barrier timeout, proceeding anyway")
        logger.info(f"[SYNC] Exchange barrier passed, ready count: {synchronizer.get_ready_count(SyncPhase.EXCHANGE_COMPLETE)}")
        ...
        # SYNC POINT 4: Wait for all nodes to be ready for next round
        logger.info("[SYNC] Waiting for all nodes to be ready for next round...")
        sync_success = synchronizer.wait_for_next_round()
        if not sync_success:
            logger.warning("[SYNC] Round barrier timeout, proceeding anyway")
        else:
            logger.info(f"[SYNC] Round barrier passed, ready for next round")
...
def run_mpls_handshake_improved(
    ...
    """
    Run improved MPLS handshake protocol with multi-peer support and message passing synchronization.
    ...
    # MESSAGE PASSING: Initialize message passing infrastructure
    mp_manager, shared_stats = init_message_passing_objects(num_nodes)
