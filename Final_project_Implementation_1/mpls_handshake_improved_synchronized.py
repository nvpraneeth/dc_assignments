...
from mpls_common import prepare_data_loaders, init_shared_objects, create_process_logger, init_synchronizer_objects
from mpls_synchronizer import create_synchronizer, SyncPhase
from utils import (
    LeNet5,
    HybridClock,
    train_local_model,
...
def improved_handshake_worker(
    ...
    synchronizer = create_synchronizer(
        ...
        logger=logger,
        timeout=30.0,  # 30 second timeout for barriers (reduced for faster failure detection)
        quorum_ratio=0.5  # 50% of nodes must be ready (more lenient to avoid deadlocks)
    )
    ...
    for round_num in range(1, r_max + 1):
        ...
        # SYNC POINT 2: Wait for all nodes to place models
        ...
        logger.info(f"[SYNC] Model placement barrier passed, ready count: {synchronizer.get_ready_count(SyncPhase.MODEL_PLACED)}")
        
        # Small delay to ensure all models are fully written to shared memory
        time.sleep(0.2)
        
        # Wait for partial models from all selected peers
        # Since we synchronized, models should be available, but we'll check with a reasonable timeout
        peer_states = {}
        wait_start = time.time()
        max_wait = 15.0  # Increased timeout since synchronization ensures models are placed
...
def run_mpls_handshake_improved(
    ...
    """
    Run improved MPLS handshake protocol with multi-peer support and synchronization.
    ...
    shared_sync, sync_locks = init_synchronizer_objects(num_nodes)
